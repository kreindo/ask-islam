...
export async function getDailyMessageCount({
  ...
  try {
    ...
    const results = await db
      ...
      .from(messageUsage)
      .where(
        and(eq(messageUsage.userId, userId), gte(messageUsage.date, today))
      );
    ...
    console.error("Failed to get daily message count from database");
...
export async function incrementDailyMessageCount({
  ...
  try {
    ...
    if (results.length === 0) {
      ...
      const currentCount = parseInt(results[0].count, 10);
      await db
        .update(messageUsage)
        .set({ count: (currentCount + 1).toString() })
        .where(eq(messageUsage.id, results[0].id));
    }

    return true;
  } catch (error) {
    console.error("Failed to increment daily message count in database");
    return false;
  }
}
